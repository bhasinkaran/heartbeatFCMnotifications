...
exports.sendNotifications = functions.database.ref('/messages/{messageid}').onWrite((event)=>{
        ...
        admin.database().ref('/users').once('value').then((data)=>{
                ...
                name=shapshotUsers[after.val().senderid]['fName']
        })
        const payload = {
                notification: {
        ...
        admin.database().ref('/chats').once('value').then((data)=>{
                if(!data.val()) return;
                const snapshot = data.val();
                const sendMessageTo = snapshot[chatid]['person1']==senderid ?snapshot[chatid]['person2'] : snapshot[chatid]['person1'];
                admin.database().ref('/users').once('value').then((data)=>{
                        if(!data.val()) return;
                        const snapshotUsers = data.val();
                        const tokens=[];
