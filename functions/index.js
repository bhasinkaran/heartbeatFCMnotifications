        ...
        admin.database().ref('/users').once('value').then((data) => {
                ...
                return axios.get(`https://pure-harbor-26317.herokuapp.com/users/${id}`)
                        .then(response => {
                                ...
                                return admin.database().ref('/chats').once('value').then((data) => {
                                        if (!data.val()) return;
                                        const snapshot = data.val();
                                        const sendMessageTo = snapshot[chatid]['person1'] === id ? snapshot[chatid]['person2'] : snapshot[chatid]['person1'];
                                    return    admin.database().ref('/users').once('value').then((data) => {
                                                ...
                                                if(snapshotUsers[sendMessageTo].token){
                                                                tokens.push(snapshotUsers[sendMessageTo].token);
                                                                console.log(tokens);
